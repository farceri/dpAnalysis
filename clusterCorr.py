...
def searchClusters(dirName, numParticles=None):
    ...
    clusterLabels = np.zeros(numParticles) # initilize all to -1
    ...
    for i in range(numParticles):
        ...
        for c in contacts[i, np.argwhere(contacts[i]!=-1)[:,0]]:
            if(particleLabel[c] != 0 and connected == False):
                clusterLabels[i] = 1
                connected = True
    # stringent condition on particles not in cluster
    for i in range(numParticles):
        if(particleLabel[i] == 0 and np.sum(contacts[i]) < 2):
            noClusterLabels[i] = 1
    np.savetxt(dirName + "/clusterLabels.dat", np.column_stack((particleLabel, clusterLabels, noClusterLabels)))
    return clusterLabels, noClusterLabels
...
def averageParticleVelSpaceCorrCluster(dirName, dirSpacing=10000):
    numParticles = int(ucorr.readFromParams(dirName, "numParticles"))
    boxSize = np.array(np.loadtxt(dirName + os.sep + "boxSize.dat"))
    minRad = np.min(np.loadtxt(dirName + os.sep + "particleRad.dat"))
    bins = np.arange(2*minRad, np.sqrt(2)*boxSize[0]/2, 2*minRad)
...
def computeClusterPT(dirName, dirSpacing=10000):
    ...
    for d in range(dirList.shape[0]):
        ...
        for i in range(numParticles):
            ...
            if(clusterLabels[i] == 1):
                pressure[d,0] += (partPressure + np.linalg.norm(vel[i]**2) * 0.5)
                activePressure[d,0] += np.dot(activeForce[i], pos[i])
            else:
                pressure[d,1] += (partPressure + np.linalg.norm(vel[i]**2) * 0.5)
